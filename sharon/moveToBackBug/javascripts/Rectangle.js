...
//		this.label=this.id;
	...
	p.ifWithin=function(pt){
		if(pt.x<=this.rectShape.getTransformedBoundingBox()[2].x&&pt.x>=this.rectShape.getTransformedBoundingBox()[0].x&&pt.y<=this.rectShape.getTransformedBoundingBox()[2].y&&pt.y>=this.rectShape.getTransformedBoundingBox()[0].y)
		return true;
		else return false;
	};
...
//debugger;
		...
		//console.log("connector's id:"+this.getConnectorID());
		//}
		//this.anchors.start.setShape({x:this.start.x, y:this.start.y});
		//this.anchors.N.setShape({x:this.N.x, y:this.N.y});
	
	};
	
	/*p.clone=function(obj){
		if(shape){
			this.duplicate=this.shape.createRect({x:this.start.x, y: this.start.y, 
						width: this.end.x-this.start.x, height:this.end.y-this.start.y, r:this.radius})
						.setFill("#def")
						.setStroke(this.property('stroke'))
						;
	...
	p.serialize=function(){
		...
		//console.log("Rectangle.serialize");
		/*var r=this.rotation*(180/Math.PI);
		if(this.start.x>this.end.x) r-=180;
		r=Math.round(r*Math.pow(10,4))/Math.pow(10,4);*/
		r=0;
		/*try{
		console.log("Rectangle.serialize()start:");
		console.log("this.writeCommonAttrs()"+this.writeCommonAttrs());
		console.log("<rect style=stroke:"+s.color+";stroke-weight:"+s.width+";fill:#def;");
	    console.log("x=:"+this.start.x +",width="+(this.end.x-this.start.x) + ",y="+this.start.y+
	                 ",height="+ (this.end.y-this.start.y) +",rx="+ this.radius +",ry="+ this.radius);
		console.log("transform=:"+ this.start.x + "," + this.start.y + ",rotate:" + r);
		console.log("<path style=fill:"+s.color);
		console.log("<text style=fill:"+s.color+";text-anchor:"+this.textAlign+"x="+this.textPosition.x+",y="+this.textPosition.y+",text:"+this.property('label'));
		console.log("Rectangle.serialize()end:");
		}
		catch(e){
			console.log("Rectangle.serialize exception catched.");
			alert(e.message);			
		}*/
		return '<g '+this.writeCommonAttrs()+'>'
			...
			+ '</g>'*/
			+ '<rect style="stroke:'+s.color+';stroke-weight:'+s.width+';fill:#def;" '
			+ 'x="' + this.start.x + '" '
