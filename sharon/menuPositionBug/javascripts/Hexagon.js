dojo.provide("dojox.diagram.Hexagon");
dojo.require("dojox.diagram.ShapeBase");

(function(){
	var ta=dojox.diagram;
	ta.Hexagon=function(figure, id){
		//summary:
		//		A hexagon shape with text annotation, extended from ShapeBase
		
		//		ta.ShapeBase.call(this, figure, id) - call ShapeBase.constructor
		
		//serialized attributes:
		//		start (x, y): the north-west point coordinate of the hexagon bounding box, keep the name = 'start' as from dojox.sketch.
		//		end (x, y): the south-east point coordinate of the hexagon bounding box, keep the name - 'end' as from dojox.sketch.
		...
		//local variables:
		//		hexagonShape(object): hexagon shape
		//		labelShape(object): text annotation
		//		property('label',this.id): set "item"+id to shape's label/text
		//		N(x,y):the north point of the hexagon bounding box
		//		NE(x,y):the north east point of the hexagon bounding box
		//		E(x,y):the east point of the hexagon bounding box
		//		S(x,y):the south point of the hexagon bounding box
		//		SW(x,y):the south-west point of the hexagon bounding box
		//		W(x,y):the west point of the hexagon bounding box
		//		anchors(object): 8 corner rectangles of the hexagon bounding box
	...
	ta.Hexagon.prototype=new ta.ShapeBase;
	var p=ta.Hexagon.prototype;
	p.constructor=ta.Hexagon;
	p.type=function(){ return 'Hexagon'; };
	p.getType=function(){ return ta.Hexagon; };
	...
	// load the hexagon from svg file
	p.apply=function(obj){
		...
		for(var i=0; i<obj.childNodes.length; i++){
			...
			if(c.localName=="text") {
				...
				//console.log("Rectangle's coordinates:start:"+this.start.x+","+this.start.y+",end:"+this.end.x+","+this.end.y+",radius:"+this.radius);
			}
		}
	};
	p.initialize=function(obj){
		...
		this.hexagonShape=this.shape.createPolyline([{x:this.start.x+(this.end.x-this.start.x)/4, y:this.start.y},{x:this.end.x-(this.end.x-this.start.x)/4, y:this.start.y},
					{x:this.end.x, y:this.start.y+(this.end.y-this.start.y)/2},{x:this.end.x-(this.end.x-this.start.x)/4, y:this.end.y},
					{x:this.start.x+(this.end.x-this.start.x)/4, y:this.end.y},{x: this.start.x, y:this.start.y+(this.end.y-this.start.y)/2},
					{x:this.start.x+(this.end.x-this.start.x)/4, y:this.start.y}])
	...
	p.destroy=function(){
		...
		this.shape.remove(this.hexagonShape);		
		this.figure.group.remove(this.shape);
		this.shape=this.labelShape=this.hexagonShape=null;
	};
	p.getBBox=function(){
	...
	p.getCenter=function(obj){
		//debugger;
		var matrix = this.hexagonShape._getRealMatrix();
		var center = dojox.gfx.matrix.multiplyPoint(matrix, this.hexagonShape.getShape().points[5].x+(this.hexagonShape.getShape().points[2].x-this.hexagonShape.getShape().points[5].x)/2, this.hexagonShape.getShape().points[5].y);
		return center;
	};
	...
	p.draw=function(obj){
		...
		this.hexagonShape.setShape([{x:this.start.x+(this.end.x-this.start.x)/4, y:this.start.y},{x:this.end.x-(this.end.x-this.start.x)/4, y:this.start.y},
					{x:this.end.x, y:this.start.y+(this.end.y-this.start.y)/2},{x:this.end.x-(this.end.x-this.start.x)/4, y:this.end.y},
					{x:this.start.x+(this.end.x-this.start.x)/4, y:this.end.y},{x: this.start.x, y:this.start.y+(this.end.y-this.start.y)/2},
					{x:this.start.x+(this.end.x-this.start.x)/4, y:this.start.y}])
					.setFill(this.property('shapeFill'))
