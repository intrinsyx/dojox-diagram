dojo.provide("dojox.diagram.Ellipse");
dojo.require("dojox.diagram.ShapeBase");

(function(){
	var ta=dojox.diagram;
	ta.Ellipse=function(figure, id){
		//summary:
		//		A ellipse shape with text annotation, extended from ShapeBase
		
		//		ta.ShapeBase.call(this, figure, id) - call ShapeBase.constructor
		
		//serialized attributes:
		//		start (x, y): the north-west point coordinate of the ellipse, keep the name = 'start' as from dojox.sketch.
		...
		//local variables:
		//		ellipseShape(object): ellipse shape
	...
	ta.Ellipse.prototype=new ta.ShapeBase;
	var p=ta.Ellipse.prototype;
	p.constructor=ta.Ellipse;

	p.type=function(){ return 'Ellipse'; };
	p.getType=function(){ return ta.Ellipse; };
	...
	p.apply=function(obj){
		...
		for(var i=0; i<obj.childNodes.length; i++){
			...
			else if(c.localName=="ellipse"){
				if(c.getAttribute('cx')!==null&&c.getAttribute('rx')!==null) {
					this.start.x=parseFloat(c.getAttribute('cx'), 10)-parseFloat(c.getAttribute('rx'), 10);
					this.end.x=parseFloat(c.getAttribute('cx'), 10)+parseFloat(c.getAttribute('rx'), 10);
					this.N.x=this.S.x=this.start.x+(this.end.x-this.start.x)/2;
					this.NE.x=this.E.x=this.end.x;
					this.SW.x=this.W.x=this.start.x;
				}
				if(c.getAttribute('cy')!==null&&c.getAttribute('ry')!==null) {
					this.start.y=parseFloat(c.getAttribute('cy'), 10)-parseFloat(c.getAttribute('ry'), 10);
					this.end.y=parseFloat(c.getAttribute('cy'), 10)+parseFloat(c.getAttribute('ry'), 10);
	...
	p.initialize=function(obj){
		...
		this.ellipseShape=this.shape.createEllipse({cx:this.start.x+(this.end.x-this.start.x)/2, cy: this.start.y+(this.end.y-this.start.y)/2, 
					rx: (this.end.x-this.start.x)/2, ry:(this.end.y-this.start.y)/2})
	...
	p.destroy=function(){
		...
		this.shape.remove(this.ellipseShape);		
		this.figure.group.remove(this.shape);
		this.shape=this.labelShape=this.ellipseShape=null;
	};
	p.getBBox=function(){
		var x=Math.min(this.start.x, this.end.x);
		var y=Math.min(this.start.y, this.end.y);
	...
	/*p.getShape=function(){
		return this.ellipseShape.getShape();
	};*/
	...
	p.getCenter=function(obj){
		var matrix = this.ellipseShape._getRealMatrix();
		var center = dojox.gfx.matrix.multiplyPoint(matrix, this.ellipseShape.getShape().cx, this.ellipseShape.getShape().cy);
		return center;
	};
	...
	p.draw=function(obj){
		...
		this.ellipseShape.setShape({cx:this.start.x+(this.end.x-this.start.x)/2, cy: this.start.y+(this.end.y-this.start.y)/2, 
					rx: (this.end.x-this.start.x)/2, ry:(this.end.y-this.start.y)/2})
					.setFill(this.property('shapeFill'))
					.setStroke({color:this.property('strokeColor'), width:this.property('strokeWidth')})
					;	
	...
	p.serialize=function(){
		...
		return '<g '+this.writeCommonAttrs()+'>'
			+ '<ellipse style="stroke:'+this.property('strokeColor')+';stroke-weight:'+this.property('strokeWidth')+';fill:'+this.property('shapeFill')+ '" '
			+ 'cx="' + this.start.x+(this.end.x-this.start.x)/2 + '" '
			+ 'cy="' + this.start.y+(this.end.y-this.start.y)/2 + '" '
			+ 'rx="' + (this.end.x-this.start.x)/2 + '" '
			+ 'ry="' + (this.end.y-this.start.y)/2 + '" '
			+ ' />'
